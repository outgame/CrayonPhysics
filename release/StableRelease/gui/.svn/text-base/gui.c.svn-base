 ...
 *@return a boolean indicating the success of the function
 */
static gboolean close_select_cb(GtkWidget *widget, Server *serv);
...
/**
 *cpSpaceBodyIteratorFunc that calls free_body_full to free the body and
 *all of its shapes
 *@param cpBody* body to be free
 *@param void* data any information that could be passed in to the function
 */
static void free_each_body(cpBody *body, void *data);
...
static gboolean select_cb(GtkWidget *widget, Gui *gui){
  ...
  serv->window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  gtk_window_set_title(GTK_WINDOW(serv->window), "Server Select Menu");
  gtk_window_set_default_size(GTK_WINDOW(serv->window), 200, 125);
  gtk_window_set_position(GTK_WINDOW(serv->window), GTK_WIN_POS_CENTER);
  gtk_container_set_border_width(GTK_CONTAINER(serv->window), 25);
  ...
  g_signal_connect(close, "clicked", G_CALLBACK(close_select_cb), serv);
  ...
  gtk_container_add(GTK_CONTAINER(serv->window), box);
  
  //show window
  gtk_widget_show_all(serv->window);

  return TRUE;
}

static gboolean server_update_cb(GtkWidget *widget, Server *serv){
  //get the text entries from the window
  ...
  if (serv->gui->sockfd != -1){
    gchar message[100];
    sprintf(message, "<CRAYON PHYSICS A++> You connected to server the server: %s and the port %s.\n", server, port);
    add_to_text(serv->gui, message);
    
    return TRUE;
  }
  //if the connection was not valid alert the user and return false
  else{
    gchar message[100];
...
static gboolean close_select_cb(GtkWidget *widget, Server *serv){
  //close window and free variables
  gtk_widget_destroy(serv->window);
  free(serv);
...
static gboolean time_handler(Gui *gui){
  ...
  //lock the mutex before updating the gui's text buffer
  pthread_mutex_lock(&((gui)->buffer_mutex));
  //check if there is something new  in the buffer
  if (gui->buff[0] != '\0'){
    //check if the text is an encrypted body
    if (gui->buff[0] < 0){
      int length, flag;
      char *p = client_decode_helper(gui->buff, &length, &flag);
      int rec [ length - 1 ];
      
      //decode message and make new bodies from the array
      client_decode(rec, p);
      gui = load_from_array(gui, rec, length, flag);
    }
    //if the text is just a message then print it to the screen
    else{
      gchar *new_tab = "> ";
...
static gboolean button_release_event_cb (GtkWidget *widget, GdkEventButton *event, Gui *gui){
  ...
  if (event->button == 1){
    //activate the body when the mouse is released
...
static gboolean motion_notify_event_cb(GtkWidget *widget, GdkEventMotion *event, Gui *gui){
  ...
  if ((event->state && GDK_BUTTON1_MASK) && (gui->current_body != NULL)){
    ...
    //add the new points to the body
...
void *client_recv1(void *gui){
  while(1){
    ...
    //loack the mutex before updating the gui's text buffer
...
Gui *load_from_array(Gui *gui, int *dec, int len, int flag) {
  //if the message was decoded for a static object
  if (flag == -128) {
  ...
  //if the message was decoded for a non-static object
  if (flag == -121) {
    ...
    //activate the drawings and clear the current body
    drawing_activate(gui->space, gui->current_body, gui->current_color);
    gui->current_body = NULL;
  }
...
void free_memory(Gui *gui){
  ...
  //getting rid of all references to the mark
  while (gui->mark){
    g_object_unref(G_OBJECT(gui->mark));
  }
  
  // function to free all of the bodies in the space
  // might need to use a chipmunk callback or another way to work around
  // using the same space that is still being updated
  //cpSpaceEachBody(gui->space, (cpSpaceBodyIteratorFunc) free_each_body, NULL);
